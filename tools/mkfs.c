...
uint ialloc(ushort type) {
    ...
    din.nlink = xshort(1);
    din.size = xint(0);
    //Lab 4 Change: set default permissions to R & W, owner to root
    din.owner = xshort(0);
...
void iappend(uint inum, void* xp, int n) {
    ...
    while (n > 0) {
        ...
        if (fbn < NDIRECT) {
            ...
            rsect(xint(din.addrs[NDIRECT + 1]), (char*)inindirect);
            if (xint(inindirect[(fbn - NDIRECT - NINDIRECT) / NINDIRECT]) == 0) {
                inindirect[(fbn - NDIRECT - NINDIRECT) / NINDIRECT] = xint(freeblock++);
                wsect(xint(din.addrs[NDIRECT + 1]), (char*)inindirect);
            }
            rsect(xint(inindirect[(fbn - NDIRECT - NINDIRECT) / NINDIRECT]), (char*)indirect);
            if (indirect[(fbn - NDIRECT - NINDIRECT) % NINDIRECT] == 0){
                indirect[(fbn - NDIRECT - NINDIRECT) % NINDIRECT] = xint(freeblock++);
                wsect(xint(inindirect[(fbn - NDIRECT - NINDIRECT) / NINDIRECT]), (char*)indirect);
            }
